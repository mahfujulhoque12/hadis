...
import { setSelectedDua } from "@/redux/feature/duaSlice";
...
const HomeLeft: React.FC<HomeLeftProps> = ({ onOptionClick }) => {
  ...
  return (
    <div className="h-[85vh] overflow-y-auto custom-scroll">
      {/* Search */}
      <div className="flex gap-2 items-center w-[90%] px-4 py-2 mt-6 rounded-full bg-[#E1EBE166] border border-[#E1EBE1]">
        <CiSearch />
        <input
        ...
        filteredDuaList.map((dua) => {
          ...
          return (
            <div key={dua.id}>
              <button
                onClick={() => {
                  if (!dua.alwaysOpen) toggle(dua.id);
                  dispatch(setSelectedDua(dua));
                  onOptionClick(); // Close sidebar when an option is clicked
                }}
